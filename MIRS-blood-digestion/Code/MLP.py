...
# filter and select only the important wavenumbers identified by SVC in X

# X = np.asarray(X.loc[:, sn_wn])
...
joblib.dump(
    scaler, 
    os.path.join("..", "Results", "data_scaler_all_wn.joblib")
    )
...
# # Name the model
model_name = 'Baseline_CNN_all_wn'
...
savedir = (outdir + r"\transfer_learning_all_wn")            
build_folder(savedir, False)
savedir = (outdir + r"\transfer_learning_all_wn\l")            
...
for train_index, test_index in kf.split(features):
    ...
    X_train, X_val, y_train, y_val = train_test_split(X_trainset,
                                        *y_trainset, test_size = validation_size, random_state = 42)
